...
MVTEC2_DIR = './datasets/mvtec2'


def load_mvtec2(category, k_shot, experiment_indx):
    ...
    assert category in mvtec2_classes
    assert k_shot in [0, 1, 5, 10]
    assert experiment_indx in [0, 1, 2]

    test_img_path = os.path.join(MVTEC2_DIR, category, 'test_public')
    train_img_path = os.path.join(MVTEC2_DIR, category, 'train')
    ground_truth_path = os.path.join(MVTEC2_DIR, category, 'test_public', 'ground_truth')
